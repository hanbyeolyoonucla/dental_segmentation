...
class ConvertToHUd(Transform):
    def __init__(self, keys, rescale_intercept_key='RescaleIntercept', rescale_slope_key='RescaleSlope'):
        self.keys = keys
        self.rescale_intercept_key = rescale_intercept_key
        self.rescale_slope_key = rescale_slope_key

    def __call__(self, data):
        d = dict(data)
        for key in self.keys:
            if key in d and f"{key}_meta_dict" in d:
                rescale_intercept = float(d[f"{key}_meta_dict"][self.rescale_intercept_key])
                rescale_slope = float(d[f"{key}_meta_dict"][self.rescale_slope_key])
                d[key] = rescale_intercept + rescale_slope * d[key].astype(np.float32)
        return d
